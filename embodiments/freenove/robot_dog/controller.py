...
class Servo:
    ...
    def __init__(self):
        self.angleMin = 18
        self.angleMax = 162
        self.PwmServo = Adafruit_PCA9685.PCA9685()
    ...
    def setServoPwm(self, channel, angle, error=10):
        ...
        if channel == '0':
            ...
            self.PwmServo.setServoPulse(16, 2500 - float((angle + error) / 0.09))
        elif channel == '9':
            self.PwmServo.setServoPulse(17, 500 + float((angle + error) / 0.09))
        elif channel == '10':
            self.PwmServo.setServoPulse(18, 500 + float((angle + error) / 0.09))
        elif channel == '11':
            self.PwmServo.setServoPulse(19, 500 + float((angle + error) / 0.09))
        elif channel == '12':
            self.PwmServo.setServoPulse(20, 500 + float((angle + error) / 0.09))
        elif channel == '13':
            self.PwmServo.setServoPulse(21, 500 + float((angle + error) / 0.09))
        elif channel == '14':
            self.PwmServo.setServoPulse(22, 500 + float((angle + error) / 0.09))
        elif channel == '15':
            self.PwmServo.setServoPulse(23, 500 + float((angle + error) / 0.09))
    ...
    def setServoAngle(self, channel, angle):
        if angle < self.angleMin:
            ...
            angle = self.angleMax
        date = self.map(angle,0,180,102,512) # Date? Why would it be called date? We need to change the name
        # print(date,date/4096*0.02)
        self.PwmServo.set_pwm(channel, 0, int(date))
    ...
    def move(self, feagi_device_id, power):
        try:
            ...
            self.setServoAngle(device_index, self.device_position)
