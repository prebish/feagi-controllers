...
with mujoco.viewer.launch_passive(m, d) as viewer:
  ...
  while viewer.is_running():
    ...
    for i, pos in enumerate(positions):
        ...
        print("test: ", test)
        if len(test) > 1:
          print("also testing this " ,f"{pos:{.3}g}")
          print(i, f"{pos:.{2}e}") # i think this part is right
        else: #this means e < 5 so we need to figure out how many digits to print?? is there an easy way???
          #CHANGE THIS PART

          #we want the first three significant digits 
          print("this needs to change: " ,pos)
          print("testing this " ,f"{pos:{.3}g}") #think this works
